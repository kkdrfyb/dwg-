...
function TryRemove([string]$path) {
    if (Test-Path $path) {
        try {
            Remove-Item $path -Recurse -Force -ErrorAction Stop
        }
        catch {
            Write-Warning "Failed to remove $path. Continuing. $($_.Exception.Message)"
        }
    }
}

Write-Host "Publishing app..." -ForegroundColor Cyan
TryRemove (Join-Path $root 'DwgOfficeToolbox.App\\bin')
TryRemove (Join-Path $root 'DwgOfficeToolbox.App\\obj')
TryRemove $publishDir
